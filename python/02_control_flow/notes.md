...
- Например:
  ...
  Т.е. на каждую итерацию внешнего цикла, приходится сразу 3 итерации внутреннего.. Получается так, что при каждой итерации внешнего цикла, внутренний должен произвести абсолютно **все** свои итерации, указанные в условии.
...
- Теперь изменим в коде оператор прерывания на оператор **`continue`**:
  ...
  ```
  Тут логика уже меняется, как и операторы)) Как только значение `i` при итерации внешнего цикла равняется значению `j` при итерации вложенного цикла - не прерываем цикл, а переходим сразу к следующей итерации (по логике программы при каждой итерации внешнего цикла, вложенный цикл совершает абсолютно **все** прописанные в нём итерации).
