...
## Конспект по теме: "Цикл `while`"

### Основные понятия:

Как известно, существуют **две** основные разновидности цикла:
- циклы, повторяющиеся **определённое количество раз**(**`for`**, счетные циклы, **counting loops**);
- циклы, повторяющиеся до наступления **определённого события**(**`while`**, условные циклы, **conditional loops**)

Цикл **`for`** замечательно работает, если заранее известно, сколько итераций потребуется сделать. Но иногда нужно, чтобы **цикл** выполнялся до наступления **некоторого события** и количество итераций в этом случае заранее оценить просто **невозможно**.
...
Двоеточие **(`:`)**  в конце строки с инструкцией **`while`** сообщает **Python**, что далее находится **блок команд**. В блок команд входят **все строки**, расположенные с отступом от строки с инструкцией **`while`**, вплоть до следующей строки без отступа.

Блок команд, который выполняется в цикле **`while`**, называется **телом цикла**.

- Программа, использующая цикл **`while`**, которая распечатывает 10 раз слово **`Здарова`**:
...
Такой код можно легко заменить циклом **`for`**, поскольку заранее известно, сколько раз необходимо выполнить тело цикла. Однако так бывает не всегда.

- Программа, которая считывает числа и выводит их квадраты, пока не будет введено **`-1`**:
  При такой постановке задачи мы не можем воспользоваться циклом **`for`** т.к. заранее неизвестно, сколько чисел будет предшествовать числу **`-1`**.
...
Далее, пока **выполняется условие цикла**, а имено пока **введённое число не равно `-1`**, мы исполняем **тело цикла**. 

В тело цикла здесь входят две команды:
1. Напечатать квадрат введённого числа
...
Цикл **`while`** очень похож на условный оператор **`if`**. Разница заключается в том, что в случае с **условным оператором** соответствующий **блок кода** будет выполнен **только 1 раз**, тогда как с циклом **`while`** **блок кода будет выполнен многократно**.
...
Для цикла **`while`** пришлось завести **самостоятельную переменную `i`** и придать ей **начальное значение**. 
...
Самостоятельно изменять значение переменной внутри тела цикла **`while`** необходимо для избежания **бесконечного цикла**, т.к. если её не изменять, то начальное значение будет одним и тем же и цикл **никогда не сможет закончиться**.

- Программа, выводящая все числа, **кратные `3`**, использующая цикл **`for`** и **`while`**:
...
Опять же, не всегда удаётся заменить цикл **`while`** с помощью цикла **`for`**. Если **заранее неизвестно количество итераций** то **необходимо использовать цикл `while` и ТОЛЬКО его**.

#### Считывание данных до стоп значения

Часто при решении задач на цикл **`while`** мы считываем **данные** до тех пор, пока пользователь не введёт некоторое значение, которое называют **стоп значением**.

- Программа, которая считывает числа и находит сумму до тех пор, пока пользователь не введёт слово **`stop`**:
...
Всегда, кроме редких случаев, цикл **`while`** должен содержать возможность **завершиться**.

Т.е. в цикле что-то должно сделать **проверяемое условие ложным**.
...
Обычно **бесконечные циклы** появляются, когда программист забывает написать программный код **внутри цикла**, который **делает проверяемое условие ложным**. В большинстве случаев следует избегать применения бесконечных циклов.
...
В данном коде мы видим, что начальное значение **`i`** никак не меняется внутри цикла и при каждой его итерации будет **меньше `10`**, так как равно **`0`**. Т.е. условие всегда **истинно** и цикл выполняется бесконечно много раз.

Бесконечные циклы можно использовать в связке с оператором прерывания **`break`**.

### Важные моменты

Цикл **`while`** получил своё название из-за характера своей работы: он выполняет некую задачу до тех пор, **пока** условие является **истинным**. 

Слово *while* на английском означает 'пока'(пока что-то выполняется/происходит и т.д.)

Цикл **`while`** называют циклом с **предусловием**, поскольку выполнению **тела цикла** предшествует **проверка условия**(сначала проверяется условие, а уже потом выполняется тело цикла).

Цикл **`while`** может не выполниться ни одного раза, если **начальное значение переменной** будет с самого начала **ложным**.

Условие в цикле **`while`**, как и в условном операторе **`if`**, может содержать логические операторы **`or`**, **`and`**, **`not`**.
  
## Конспект по теме "Обработка цифр числа с помощью циклов `for` и `while`"

### Основные понятия

#### Обработка цифр числа с помощью цикла `while`

При изучении целых чисел(типа данных **`int`**) говорилось про **операцию целочисленного деления `//`** и **операцию нахождения остатка от деления одного целого числа на другое `%`**.
...
- Программа, которая определяет, есть ли в числе **`num`** цифра **`7`**
...
Формула для получения любого числа(уже известная из конспектов про арифметические операторы):

**`(num // 10 ** (n - i)) % 10

где **`n`** - количество разрядов числа, **`i`** - номер цифры числа(слева направо, начиная с **`1`**).

Для того, чтобы пройтись по всем цифрам числа, необходимо лишь знать количество разрядов **`n`** этого числа.

- Чтобы найти это самое количество, можно использовать следующий способ:

  ```python
  n = len(str(num))
  ```
  
Другими словами, можно сказать, что тут находится "длина" числа.

Если все разряды числа пронумеровать от **`1`** до **`n`** включительно, то получится рассмотреть все цифры числа слева направо следующим образом.
...
## Конспект по теме: "Операторы - `break`, `continue` и необязательный блок - `else`"

### Основные понятия

#### Оператор прерывания цикла `break`

Иногда бывает необходимо **прервать выполнение цикла преждевременно**(т.е. до его завершения).

Оператор **`break`** **прерывает** ближайший цикл **`for`** или **`while`**.

- Пример программы, проверяющей число на простоту с оператором **`break`**:
...
**Оператор прерывания цикла `break`** позволяет **ускорять** программы, т.к. избавляет от лишних итераций.

Напишем программу, которая использует цикл **`for`**, считывает 10 чисел и суммирует их до тех пор, пока не обнаружит отрицательное число.
...
- Пример кода с сигнальной меткой и **`break`**:
  ...
  Как только цикл встречает цифру 7, значение сигнальной метки меняется и цикл прерывается с помощью команды **`break`**.
  Можно и не прерывать цикл преждевременно, а дождаться его естественного завершения, но в этом случае будет совершаться лишняя работа, и в случае, если число оооочень большое, программа станет работать гораздо медленнее.

#### Бесконечные циклы

В предыдущем конспекте о циклах, было сказано о цикле, который не имеет возможности завершиться и называется **бесконечным циклом**. 
...
**Бесконечный цикл** продолжает повторяться до тех пор, пока программа не будет прервана. Изучив **оператор `break`**, я получил **механизм прерывания бесконечных циклов**.

Возможно, может показаться, что бесконечные циклы лишены смысла, но это не совсем так. 

Например, можно написать программу, которая запускается и работает, постоянно принимая запросы на обслуживание.

- Код такой программы будет выглядеть так:
...
- Например:
  ...
  while True:
      if условие 1: # условие для остановки цикла
          break
      if условие 2: # ещё условие для остановки цикла
          break
      if условие 3: # ещё одно условие))
          break
  ```
...
Бесконечные циклы можно и иногда даже нужно использовать, т.к. они могут быть очень полезны. Главное помнить, что необходимо убедиться в том, что цикл в нужный момент будет прерван, чтобы он не убивал программу бесконечными итерациями.
...
- Напишем программу, которая выводит все числа от `1` до `100`, кроме чисел `7`, `17`, `29`, `78`:
...
#### Графическое представление операторов `break` и `continue`

<img width="693" height="258" alt="image" src="https://github.com/user-attachments/assets/ac4a1d22-57cb-4fb7-919d-e79b78e05f8d" />
...
**`блок кода2`**, указанный в **`else`** будет выполнен, когда **цикл `while` или `for` завершается штатно и самостоятельно**.

Можно подумать, как это может быть полезным??? Ведь можно сделать то же самое, вынеся **`блок кода2`** сразу после цикла **`while`** или **`for`** без использования **`else`**:
...
А разница в том, что если слово **`else`** отсутствует в описании цикла, то **`блок кода2`** будет выполняться после завершения цикла в любом случае и несмотря ни на что. Т.е. он будет не привязан к работе цикла абсолютно никак по сути-то.

А вот если слово **`else`** присутствовать будет, то **`блок кода2`** будет выполняться только в том случае, если цикл завершается самостоятельно, т.е. без использования оператора прерывания **`break`**.
...
- Например:
  ...
  while n > 0:
      ...
      print('Цикл завершён.')
  ```
  Данный цикл повторяется до тех пор, пока истинно условие **`n > 0`**. Поскольку цикл завершается самостоятельно, то блок кода в инструкции **`else`** будет выполнен.
  
  Таким образом результатом выполнения данной программы будут строки:
...
- Рассмотрим цикл с оператором прерывания **`break`**:
  ...
  Т.к. этот цикл завершается преждевременно, то блок кода в инструкции **`else`** не будет выполнен.
  ...
  Может показаться, что инструкция **`else`** в циклах **`while`** и **`for`** не совсем соответствует тому, что реально происходит. Гвидо Ван Россум - **создатель Python**, сказал, что если бы он проектировал язык **Python** заново, то избавился бы от **`else`** в циклах.

Напишем программу, которая определяет, содержит ли введённое пользователем число цифру 7.
...
- Вместо программного кода, написанного ранее:
  ...
  Можно использовать более оптимальный вариант:
...
Блок кода **`else`** в циклах встречается не так часто на практике, но если обнаружится ситуация, в которой его применение оправдано, то **`else`** нужно смело использовать.

## Конспект по теме: "Вложенные циклы"

### Основные понятия

#### Вложенные циклы

**Вложенный цикл** - это цикл, расположенный в **ещё одном цикле**. 

Обыкновенные часы служат хорошим примером вложенного цикла. Секундная, минутная и часовая стрелки вращаются вокруг циферблата. Часовая стрелка смещается всего на 1 шаг для каждых 60 шагов минутной стрелкой, а секундная стрелка в свою очередь должна сделать 60 шагов для 1 шага минутной стрелки. Это означает, что для каждого **полного** оборота часовой стрелки(12 шагов), минутная стрелка делает 720 шагов.

- Цикл, который частично моделирует электронные часы:

  ```python
  for seconds in range(60):
      print(seconds)
  ```
  Он показывает секунды от 0 до 59.

  Можно добавить переменную **`minutes`** и вложить цикл написанный выше внутрь ещё одного цикла, который повторяется 60 раз:
  ...
  ```
  Для того, чтобы закончить модель, можно добавить ещё одну переменную для подсчёта часов:
  ...
  Самый внутренний цикл сделает 60 итераций для каждой итерации среднего цикла.

  Средний цикл сделает 60 итераций для каждой итерации самого внешнего цикла.

  Когда внешний цикл сделает 24 итерации, средний сделает **`24 * 60 = 1440`** итераций, а самый внутренний цикл сделает **`24 * 60 * 60 = 86400`** итераций!

Пример имитационной моделей часов подводит к нескольким моментам, имеющим отношение к вложенным циклам:
...
Можно вкладывать друг в друг циклы как **`for`**, так и **`while`**
...
- Например:
  ...
  for i in range(3):
      for j in range(3):
          print(i, j)
  ```
  Результатом выполнения будут 9 строк:
  ...
  0 1
  0 2
  # 2 итерация внешнего цикла
  1 0
  1 1
  1 2
  # 3 итерация внешнего цикла
  2 0
  2 1
  2 2
  ```

  Т.е. на каждую итерацию внешнего цикла, приходится сразу 3 итерации внутреннего..

  Получается так, что при каждой итерации внешнего цикла, внутренний должен произвести абсолютно **все** свои итерации, указанные в условии.
...
- Изменим код, добавив во вложенный цикл **условный оператор** с оператором **`break`**:
  ...
  Результатом выполнения данного кода будут 3 строки:
  ...
  2 1
  ```
  Здесь мы уже видим, что 1 итерация у нас сразу же прерывается, т.к. будет `0 == 0`, значит сразу переходим ко второй итерации внешнего цикла - выводится `1 и 0`, т.к. они не равны, а далее снова срабатывает **`break`** и мы переходим уже к 3 итерации внешнего цикла, где `2 != 0 и 2 != 1`, поэтому выводим 2 итерации внутреннего `2 0` и `2 1`.

- Теперь изменим в коде оператор прерывания на оператор **`continue`**:
  ...
  for i in range(3):
      for j in range(3):
          ...
          print(i, j)
  ```
  Результатом выполнения этого кода будут 6 строк:

  ```python
  # 1 итерация внешнего цикла
  0 1 # 2 итерация вложенного
  0 2 # 3 итерация вложенного
  # 2 итерация внешнего цикла
  1 0 # 1 итерация вложенного
  1 2 # 2 итерация вложенного
  # 3 итерация внешнего цикла
  2 0 # 1 итерация вложенного
  2 1 # 2 итерация вложенного
  ```
  Тут логика уже меняется, как и операторы))

  Как только значение `i` при итерации внешнего цикла равняется значению `j` при итерации вложенного цикла - не прерываем цикл, а переходим сразу к следующей итерации (по логике программы при каждой итерации внешнего цикла, вложенный цикл совершает абсолютно **все** прописанные в нём итерации).
...
- Пример с сигнальной меткой:
  ...
  for i in range(3):
      ...
      for j in range(3):
          ...
          print(i, j)
  ```
  В данном случае у нас в выводе не выведется ничего, т.к. внешний цикл будет прерван уже на 1 итерации вложенного, когда `i` равное 0, будет сравнена с `j` равной 0 - `i == j` -> значение сигнальной метки меняется на `True`, затем прекращается итерация вложенного цикла и потом при следующей итерации внешнего, когда будет проверка значения сигнальной метки - итерация внешнего цикла завершится.

#### Примеры решения задач

Один интересный способ узнать о работе вложенных циклов состоит в их использовании для вывода на экран **комбинаций символов**. 

- Например, необходимо напечатать звёздочки в виде прямоугольной таблицы:
  ...
  ******
  ...
  Таблица, состоящая из звёздочек состоит из **`8`** строк и **`6`** столбцов.

  Приведённый ниже фрагмент кода можно использовать для вывода **одной** строки звёздочек:
  ...
  Для того, чтобы завершить весь вывод таблицы звёздочек, необходимо выполнить данный выше цикл **`8`** раз.

  Для этого поместим этот цикл в ещё один цикл, который делает 8 итераций:
  ...
  Внешний цикл делает **`8`** итераций.

  Во время каждой итерации этого цикла внутренний цикл делает **`6`** итераций.

  Обращаем внимание на то, что в строке 4, после того, как все итерации внутреннего цикла завершены, мы вызываем функцию **`print`** - это необходимо для того, чтобы в конце каждой строки был переход на следующую, ведь в принте вложенного цикла операнд **`end`** ничему не равен, а следовательно строка не будет смещаться и звездочки будут все на одной строке, если не поставить в конце обычный **`print()`**.
...
Зная, что решения (корни) уравнения являются натуральными (целыми) числами, несложно написать программу, содержащую вложенный цикл и перебирающую все возможные значения переменных.

##### Решение задач

Задача 1. Найдите все пары натуральных чисел (и их количество), являющиеся решением уравнения *12x + 13y = 777*.

Решение. Поскольку по условию числа *x* и *y* являются **натуральными**, то *x* <= 64, *y* <= 59(при *x* = 65, 12 * 65 - 780, что больше, чем 777, поэтому *x* <= 64, т.к. *y* **не может быть отрицательным по условию**; аналогично получается, что *y* <= 59)
